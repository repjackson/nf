...
if Meteor.isClient
    ...
    Template.wp.onCreated ->
        @autorun -> Meteor.subscribe 'wp_facets',
    ...
    Template.wp_order.onCreated ->
        @autorun => Meteor.subscribe 'product_by_wp', @data, ->
    Template.wp_order.helpers
        related_product: -> 
            Docs.findOne
                model:'product'
                slug:@_product
...
if Meteor.isServer 
    Meteor.methods
        ...
        wp_meta: (doc_id)->
            wp_order = Docs.findOne doc_id
            split = wp_order.Ean_Code.split('/')
            # console.log split[4]
            
            
            converted = moment(wp_order.Txn_Timestamp, ["DD/MM/YYYY HH:mm:ss"]).toDate()
            Docs.update doc_id, 
                $set:
                    _product:split[4]
        ...
        parse_wp: (parsed_results)->
            ...
            for item in parsed_results.data
                ...
                if found_item 
                    ...
                    Meteor.call 'wp_meta', found_item._id, ->
                else 
                    item.model = 'wp_order'
                    new_id = Docs.insert item
                    Meteor.call 'wp_meta', new_id, ->
                # console.log item.Txn_Timestamp, converted

    Meteor.publish 'wp_total', (
        product_search=''
        picked_products=[]
        picked_month=null
        picked_weeknum=null
        ...
        Counts.publish this, 'wp_total', Docs.find(match)
        return undefined


    Meteor.publish 'product_by_wp', (wp_order)->
